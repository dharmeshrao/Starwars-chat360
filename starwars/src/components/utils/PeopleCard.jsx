    ...
    <div className="w-4/5 p-4 bg-white m-auto rounded-xl drop-shadow-lg flex flex-col gap-2">
      ...
      <div className="bg-purple-100 bg-opacity-40 rounded-xl h-8 flex p-6 items-center">
        ...
        {starships &&
          starships?.length > 0 &&
          starships.map((e, i) => (
            <button key={i} className="bg-white ml-4 hover:bg-gray-100 text-purple-600 font-semibold py-1 px-4 border border-purple-600 rounded shadow">
              {i + 1}
            </button>
          ))}
      </div>
